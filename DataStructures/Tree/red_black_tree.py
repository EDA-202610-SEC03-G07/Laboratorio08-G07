...
def insert_node(root, key, value):
    if root is None:
        ...
        if key < rbt.get_key(root): #caso 1 - el key es menor que la llave del root , por lo tanto para la izquierda
            root["left"] = insert_node(root["left"], key, value)
        elif key > rbt.get_key(root): #caso 2 - el key es mayor que la llave del root, por lo tanto para la derecha
            root["right"] = insert_node(root["right"], key, value)
        else:
            root["value"] = value
        right_is_red = rbt.is_red(root["right"])
        left_is_red = rbt.is_red(root["left"])
        if right_is_red and not left_is_red:
            root = rotate_left(root)
        right_is_red = rbt.is_red(root["right"])
        left_is_red = rbt.is_red(root["left"])
        if left_is_red and rbt.is_red(root["left"]["left"]):
